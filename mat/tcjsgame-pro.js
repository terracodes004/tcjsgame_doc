...
class Display {
    ...
    start(width = 480, height = 270, no = document.body) {
        ...
        this.interva = setInterval(() => move.resize(TCJSgameGameArea, this.canvas.width, this.canvas.height), 20);
        this.mapWidth = this.canvas.width;
        this.mapHeight = this.canvas.height;
    ...
    perform(){
        ...
        this.interval = TCJSgameGameAreaToIncreasePerformance()
    }
}
...
class Sound {
    constructor(src) {
        ...
        setInterval(()=>{this.sound.volume = this.volume}, 20)
        
    }

    play() {
        let sound = this.sound
        this.sound.oncanplay = function(){
            try{
                sound.play()
...
let move ={
    ...
    bound : function(id){
        ...
        if (id.x >= display.canvas.width - id.width){
...
class TileMap {
    ...
    show(){
        ...
        this.map.forEach((y)=>{
            ...
            y.forEach((x)=>{
                if(x){
                    this.tile[x].width = this.tileWidth
                    this.tile[x].height = this.tileHeight
                    this.tile[x].y = yy
                    this.tile[x].height = this.tileHeight
                    this.tile[x].width = this.tileWidth
                    this.tile[x].x = xx
                    currentTile = new Tile(txx, tyy, x, this.tile[x])
                    this.tileList.push(currentTile)
                    this.tileComm.push({x : currentTile, scene : this.scene})
                    this.tileComm.forEach(component => {
                    if(component.scene == this.render.scene){
...
const TCJSgameGameArea = new Component(display.canvas.width,display.canvas.height, "black", 0,0)
...
function TCJSgameGameAreaToIncreasePerformance(){
    ...
    Mouse.x = mouse.x
        ...
        TCJSgameGameArea.width = display.canvas.width
        TCJSgameGameArea.height = display.canvas.height
        comm.forEach(component => {
        ...
        return requestAnimationFrame(TCJSgameGameAreaToIncreasePerformance)
}

class ParticleSystem {
    constructor(x, y, color, count, size, speed, life, gravity = 0) {
        this.particles = [];
